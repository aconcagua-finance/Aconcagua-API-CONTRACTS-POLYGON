...
export function getParsedEthersError(error: EthersError): ReturnValue {
  ...
  if (nestedLevelEthersError !== undefined) {
    const nestedLevelKnownErrorCode = getNestedLevelKnownError(nestedLevelEthersError);
  ...
  if (topLevelEthersError.transaction !== undefined && topLevelEthersError.receipt !== undefined) {
