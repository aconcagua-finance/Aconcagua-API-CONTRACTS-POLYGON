...
export function getUnpredictableGasLimitError(ethersError: EthersError): ReturnValue | undefined {
  ...
  if (
    ...
    ethersError.error.error.error.code === NESTED_ETHERS_ERROR_CODES.TRANSACTION_UNDERPRICED &&
    ...
    try {
      ...
      if (bodyObject.error !== undefined && bodyObject.error.message !== undefined) {
        if (bodyObject.error.message.includes('gas required exceeds allowance (0)')) {
          return {
        ...
        if (
          bodyObject.error.message.includes('max priority fee per gas higher than max fee per gas')
        ) {
          return {
            errorCode:
        ...
        if (bodyObject.error.message.includes('max fee per gas less than block base fee')) {
          return {
            errorCode: RETURN_VALUE_ERROR_CODES.MAX_FEE_PER_GAS_LESS_THAN_BLOCK_BASE_FEE,
            context: undefined,
          };
        }
      }
