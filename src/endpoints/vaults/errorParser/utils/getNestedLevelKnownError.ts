...
export function getNestedLevelKnownError(ethersError: NestedEthersError): ReturnValue | undefined {
  ...
  if (errorCodeMessage !== undefined) {
    const errorWhileFormattingOutputFromRPC = getErrorWhileFormattingOutputFromRPCError(
      errorCode,
      errorCodeMessage
    );
