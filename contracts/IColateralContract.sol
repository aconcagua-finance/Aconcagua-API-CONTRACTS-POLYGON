...
interface IColateralContract {
    ...
    function setStartTimePeriod(uint _startTimePeriod) external;

    function setTokenAddress(address _usdcTokenAddress, string calldata _tokenSymbol) external;

    function setWithdrawalLimitPerPeriod(uint256 _withdrawalLimitPerPeriod, string calldata _tokenSymbol) external;
