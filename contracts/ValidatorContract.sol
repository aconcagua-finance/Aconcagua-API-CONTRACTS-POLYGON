...
contract ValidatorContract is IValidatorContract {
    ...
    function swapExactInputsChecks(
        ...
        if (swapParams.tokenOut == USDC) {
            ...
            tokenOutDecimals = 6; // USDT has 6 decimals
        } else {
            revert TokenOutError();
        }

        // Convert price to 6 decimals
        priceIn6Decimal = price / (10**8) * (10**6);

        // Calculate oracleAmountOut in 6 decimals
        uint256 oracleAmountOut6Decimal = (swapParams.params.amountIn * uint256(priceIn6Decimal)) / (10**(tokenInDecimals));

        // Calculate amountOutMinimum in 6 decimals
        uint256 amountOutMinimum6Decimal = swapParams.params.amountOutMinimum * (10**6) / (10**tokenOutDecimals);

        // Calculate the minimum allowed amount considering slippage in 6 decimals
        uint256 minAllowedAmount6Decimal = (oracleAmountOut6Decimal * oracleSlippage) / 100;

        // Compare amountOutMinimum with minAllowedAmount
        if (amountOutMinimum6Decimal < minAllowedAmount6Decimal) {
